...
if (!empty($_GET['id'])){
    setcookie("service_id",serialize($_GET['id']),time() + 3600);
    $service_id = $_GET['id'];
} else if (isset($_COOKIE["service_id"])){
    $service_id = unserialize($_COOKIE["service_id"]);
} else {
...
<body>
    ...
    <div class="container mb-5">
        <form method="post" action="confirm_book.php?<?php echo 'year='.$year.'&week='.$week.'&day='.$day ?>">
        ...
        <div class="row-cols-lg-auto d-flex justify-content-around">
            ...
            echo "<div class='col-lg-7'>";
                ...
                switch($day) {
                    ...
                    case 4:
                    case 5:
                        hours($open_hours[0][0], $open_hours[0][1], $now, $date, $service_id);
                        break;
                    case 6:
                        hours($open_hours[1][0], $open_hours[1][1], $now, $date, $service_id);
                        break;
                }
            ?>
            </div>
...
function hours(Datetime $open, DateTime $close, DateTime $now, DateTime $date,$service_id){
        $time_needed = Booking::duration($service_id);
        $time_needed = DateInterval::createFromDateString("$time_needed minutes");
        $result = Booking::getBooking($open->format("Y-m-d H:i:s"),$close->format("Y-m-d H:i:s"));
        $clone = clone $close;
                ...
                if ($last_book >= $date) {
                    ...
                    if (((int)$free_time->format("%h")*60 + (int)$free_time->format("%i")) >= $time_needed->format("%i")) {
                        echo "<label class='btn btn-primary btn-sm m-1'>" .
                            "<input type='radio' name='time' value='$str'>$str" .
                            "</label>";
                        $date->modify('10 minutes');
                        $row++;
                    } else {
